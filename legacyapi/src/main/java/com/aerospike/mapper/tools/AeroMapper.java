...
public class AeroMapper implements IAeroMapper {
    ...
    private AeroMapper(@NotNull IAerospikeClient client) {
        this.mClient = client;
        this.recordLoader = new AerospikeRecordLoader(client);
    ...
    public static class Builder extends AbstractBuilder<AeroMapper> {
        public Builder(IAerospikeClient client) {
            ...
            PolicyCache.getInstance().setDefaultPolicies(client);
    ...
    @SafeVarargs
    @Override
    public final <T> void save(@NotNull T... objects) throws AerospikeException {
    ...
    public <T> void save(@NotNull WritePolicy writePolicy, @NotNull T object, String... binNames)
        ...
        Bin[] bins = toBins(entry, object, writePolicy.recordExistsAction != RecordExistsAction.REPLACE, binNames);
    ...
    private <T> WritePolicy generateWritePolicyFromObject(T object) {
        ...
        WritePolicy writePolicy = PolicyCache.getInstance().getEffectiveWritePolicy(clazz, mClient.getWritePolicyDefault());
    ...
    private <T> T read(Policy readPolicy, @NotNull Class<T> clazz, @NotNull Key key, @NotNull ClassCacheEntry<T> entry,
        ...
        if (readPolicy == null) {
            readPolicy = PolicyCache.getInstance().getEffectiveReadPolicy(clazz, mClient.getReadPolicyDefault());
        }
        ...
        if (record == null) {
            ...
            try {
                ThreadLocalKeySaver.save(key, key.userKey);
    ...
    private <T> T[] readBatch(BatchPolicy batchPolicy, @NotNull Class<T> clazz, @NotNull Key[] keys,
        ...
        if (batchPolicy == null) {
            batchPolicy = PolicyCache.getInstance().getEffectiveBatchPolicy(clazz, mClient.getBatchPolicyDefault());
        }
        ...
        for (int i = 0; i < records.length; i++) {
            if (records[i] == null) {
                ...
                try {
                    ThreadLocalKeySaver.save(keys[i], keys[i].userKey);
                    Map<String, Object> map = recordToMap(entry, keys[i], records[i]);
                    T result = entry.constructAndHydrate(map);
                    entry.setGenerationValue(result, records[i].generation);
                    results[i] = result;
                } finally {
                    ThreadLocalKeySaver.clear();
                }
    ...
    public <T> boolean delete(WritePolicy writePolicy, @NotNull Class<T> clazz, @NotNull Object userKey)
        ...
        if (writePolicy == null) {
            writePolicy = PolicyCache.getInstance().getEffectiveWritePolicy(clazz, mClient.getWritePolicyDefault());
    ...
    public boolean delete(WritePolicy writePolicy, @NotNull Object object) throws AerospikeException {
        ...
        if (writePolicy == null) {
            writePolicy = PolicyCache.getInstance().getEffectiveWritePolicy(object.getClass(), mClient.getWritePolicyDefault());
            if (entry.getDurableDelete() != null) {
                writePolicy = new WritePolicy(writePolicy);
                writePolicy.durableDelete = entry.getDurableDelete();
            }
    ...
    public <T> void find(@NotNull Class<T> clazz, Function<T, Boolean> function) throws AerospikeException {
        ...
        try (RecordSet recordSet = mClient.query(null, statement)) {
            // TODO: set the policy (If this statement is thought to be useful, which is dubious)
            T result;
            while (recordSet.next()) {
                result = clazz.getConstructor().newInstance();
                Map<String, Object> map = recordToMap(entry, recordSet.getKey(), recordSet.getRecord());
                entry.hydrateFromMap(map, result);
    ...
    public <T> void scan(ScanPolicy policy, @NotNull Class<T> clazz, @NotNull Processor<T> processor,
        ...
        ClassCacheEntry<T> entry = MapperUtils.getEntryAndValidateNamespace(clazz, this);
        if (policy == null) {
            policy = PolicyCache.getInstance().getEffectiveScanPolicy(clazz, mClient.getScanPolicyDefault());
        }
        ...
        try {
            mClient.scanAll(policy, namespace, setName, (key, record) -> {
                Map<String, Object> map = recordToMap(entry, key, record);
                T object = mappingConverter.convertToObject(clazz, map);
    ...
    public <T> void query(QueryPolicy policy, @NotNull Class<T> clazz, @NotNull Processor<T> processor, Filter filter) {
        ClassCacheEntry<T> entry = MapperUtils.getEntryAndValidateNamespace(clazz, this);
        if (policy == null) {
            policy = PolicyCache.getInstance().getEffectiveQueryPolicy(clazz, mClient.getQueryPolicyDefault());
        }
        Statement statement = new Statement();
        ...
        try (RecordSet recordSet = mClient.query(policy, statement)) {
            while (recordSet.next()) {
                Map<String, Object> map = recordToMap(entry, recordSet.getKey(), recordSet.getRecord());
                T object = mappingConverter.convertToObject(clazz, map);
                if (!processor.process(object)) {
    ...
    private Policy getPolicyByClassAndType(Class<?> clazz, PolicyType policyType) {
        switch (policyType) {
        case READ:
            return PolicyCache.getInstance().getEffectiveReadPolicy(clazz, mClient.getReadPolicyDefault());
        case WRITE:
            return PolicyCache.getInstance().getEffectiveWritePolicy(clazz, mClient.getWritePolicyDefault());
        case BATCH:
            return PolicyCache.getInstance().getEffectiveBatchPolicy(clazz, mClient.getBatchPolicyDefault());
        case SCAN:
            return PolicyCache.getInstance().getEffectiveScanPolicy(clazz, mClient.getScanPolicyDefault());
        case QUERY:
            return PolicyCache.getInstance().getEffectiveQueryPolicy(clazz, mClient.getQueryPolicyDefault());
        default:
    ...
    /**
     * Builds an Aerospike map representation of an object ready for writing, excluding non-bin key
     * fields and applying optional bin name filtering.
     */
    @SuppressWarnings({"unchecked", "rawtypes"})
    static Bin[] toBins(ClassCacheEntry<?> entry, Object instance, boolean allowNullBins, String[] binNames) {
        Map<String, Object> map = entry.getMap(instance, false);
        // Exclude key field if it is not stored as a bin
        String keyFieldName = entry.getKeyFieldName();
        if (keyFieldName != null && !entry.isKeyFieldStoredAsBin()) {
            map.remove(keyFieldName);
        }
        List<Bin> binList = new ArrayList<>();
        for (Map.Entry<String, Object> e : map.entrySet()) {
            if (!containsBinName(binNames, e.getKey())) continue;
            Object value = e.getValue();
            if (value == null && !allowNullBins) continue;
            if (value instanceof TreeMap<?, ?>) {
                TreeMap<?, ?> treeMap = (TreeMap<?, ?>) value;
                binList.add(new Bin(e.getKey(), new ArrayList(treeMap.entrySet()), MapOrder.KEY_ORDERED));
            } else {
                binList.add(new Bin(e.getKey(), Value.get(value)));
            }
        }
        return binList.toArray(new Bin[0]);
    }

    private static boolean containsBinName(String[] names, String thisName) {
    ...
    /**
     * Converts an Aerospike Record to a bin map, injecting the user key value for classes
     * that use sendKey=true (keyAsBin=false).
     */
    static <T> Map<String, Object> recordToMap(ClassCacheEntry<T> entry, Key key, Record record) {
        Map<String, Object> map = new HashMap<>(record.bins);
        if (!entry.isKeyFieldStoredAsBin()) {
            String keyFieldName = entry.getKeyFieldName();
            if (keyFieldName != null && key != null && key.userKey != null) {
                map.put(keyFieldName, key.userKey.getObject());
            }
        }
        return map;
    }
