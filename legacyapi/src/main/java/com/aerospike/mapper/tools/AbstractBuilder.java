    ...
    public static class AeroPolicyMapper<T extends IBaseAeroMapper> {
        ...
        public AbstractBuilder<T> forClasses(Class<?>... classes) {
            for (Class<?> thisClass : classes) {
                PolicyCache.getInstance().setSpecificPolicy(policyType, thisClass, policy);
            }
            return builder;
        }

        public AbstractBuilder<T> forThisOrChildrenOf(Class<?> clazz) {
            PolicyCache.getInstance().setChildrenPolicy(this.policyType, clazz, this.policy);
            return builder;
        }

        public AbstractBuilder<T> forAll() {
            PolicyCache.getInstance().setDefaultPolicy(policyType, policy);
            return builder;
        }
    }
