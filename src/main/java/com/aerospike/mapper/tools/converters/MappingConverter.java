...
public class MappingConverter {
    ...
    public MappingConverter(IBaseAeroMapper mapper, IAerospikeClient client) {
        this.mapper = mapper;
        this.client = client;
    }
    ...
    public void resolveDependencies(ClassCacheEntry<?> parentEntity) {
        ...
        BatchPolicy batchPolicy = parentEntity == null ? client.getBatchPolicyDefault() : parentEntity.getBatchPolicy();
        ...
        while (!deferredObjects.isEmpty()) {
            ...
            Record[] records = client.get(batchPolicyClone, keys);
            ...
            reactorClient
                    ...
                    .map(keyRecord -> {
                        try {
                            DeferredObjectLoader.DeferredObjectSetter thisObjectSetter = deferredObjects.get(count);
                            ThreadLocalKeySaver.save(keyRecord.key);
                            return convertToObject((Class)thisObjectSetter.getObject().getType(), keyRecord.record, classCaches[count], false);
                        } catch (ReflectiveOperationException e) {
                            throw new AerospikeException(e);
                        } finally {
                            ThreadLocalKeySaver.clear();
